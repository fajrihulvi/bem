...
class Jenis_kegiatan extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['record'] = $this->db->order_by('id', 'DESC')->get('jenis_kegiatan')->result_array();
		$this->template->load('admin/template', 'master/jenis_kegiatan/index', $data);
	}

	public function tampilubah()
	{
		echo json_encode($this->db->get_where('jenis_kegiatan', ['id'=>$_POST['id']])->row());
	}
	...
	public function tambah()
	{
		if(isset($_POST['submit'])){
			...
			if($this->db->insert('jenis_kegiatan', $data)){
	...
	public function edit()
	{
		if(isset($_POST['submit'])){
			...
			if($this->db->update('jenis_kegiatan', $data, ['id' => $id])){
	...
	public function hapus($id)
	{
		...
		if($this->db->delete('jenis_kegiatan', ['id' => $id])){
